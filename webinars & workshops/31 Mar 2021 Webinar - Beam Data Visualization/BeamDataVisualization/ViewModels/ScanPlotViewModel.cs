...
namespace BeamDataVisualization.ViewModels
{
    public class ScanPlotViewModel : BindableBase
    {
        ...
        public DelegateCommand ExportScansCommand { get; private set; }
        ...
        public ScanPlotViewModel(IEventAggregator eventAggregator)
        {
            ...
            ExportScansCommand = new DelegateCommand(OnExportScans);
        }

        private void OnExportScans()
        {
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.Title = "Choose saving path for file";
            if(sfd.ShowDialog() == true)
            {
                foreach(var scan in BeamScans)
                {
                    var filepath = Path.Combine(Path.GetDirectoryName(sfd.FileName),$"{scan.DisplayTxt}.csv");
                    using(StreamWriter sw = new StreamWriter(filepath))
                    {
                        sw.WriteLine("Position,Dose");
                        foreach(var data in scan.BeamDataPoints)
                        {
                            sw.WriteLine($"{data.Position},{data.DoseValue}");
                        }
                    }
            ...
            foreach (var beam in plan.Beams)
            {
                ...
                foreach (var depth in depths)
                {
                    ...
                    var beamscan = new BeamScanModel
                    {
                        ...
                        FieldX = fieldX,
                        FieldY = fieldY,
                        DisplayTxt = $"Crossline {fieldX:F0}x{fieldY:0} - {depth:F1}"
                    };
                    ...
                    var diagScan = new BeamScanModel
                    {
                        ...
                        FieldX = fieldX,
                        FieldY = fieldY,
                        DisplayTxt = $"Diagonal {fieldX:F0}x{fieldY:F0} - {depth:F1}"
                    };
                ...
                var pdd_scan = new BeamScanModel
                {
                    ...
                    FieldX = fieldX,
                    FieldY = fieldY,
                    DisplayTxt = $"PDD {fieldX:F0}x{fieldY:F0}"
                };
